#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Drive the 0.3.7 package build on 192.168.8.6 and pull the artifacts home.
...
ARTIFACTS = [
    "dsh-raganything-kb-0.3.7-portable.tar.gz",
    "dsh-raganything-kb-0.3.7.tgz",
]
EXTRAS = ["dsh-raganything-kb-0.3.7-manifest.json", "dsh-raganything-kb-0.3.7-README.md"]

BUILD_SCRIPT = r"""
mkdir -p "$HOME/%(d)s"
cd "$HOME/%(d)s"
rm -f build037.log
python3 -u build_pkg037.py 2>&1 | tee build037.log
...
def main():
    ...
    if args.mode == "build":
        sftp = cli.open_sftp()
        scripts = [os.path.join(BASE, "tools", "build_pkg037.py"),
        ...
        for lp in scripts:
            ...
            sftp.put(lp, rp)
            print("uploaded %-22s %d bytes" % (os.path.basename(lp), os.path.getsize(lp)))
        sftp.close()
        ...
        M = "/home/lgsj/dsh-raganything-kb-0.3.7-portable/manifest.json"
        R = "/home/lgsj/dsh-raganything-kb-0.3.7-portable/README.md"
