#!/usr/bin/env python3
"""Switch the 12 dsh-raganything-kb installs on 8.6 from 0.3.12-sharedkb to
0.3.13-sharedkb.  Runs ON the host as lgsj (no sudo needed: everything is under
/home/lgsj).

Why not `pnpm install`: the host has no public egress, and a resolve step would
also wipe the `.bak-*` backups sitting inside node_modules.  The 0.3.13 payload is
byte-identical to 0..12 except `server.py` (the @-scope fix) and `package.json`
(version), so a surgical sync is enough:
  1. profile package.json : dep 0.3.12 -> 0.3.13   (backup first)
  2. installed package.json: take the 0.3.13 one from the tarball (backup first)
  3. installed RELEASE-NOTES.md: refresh
Nothing else is touched; `.bak-*` backups are left in place.

Usage:  python3 deploy_profiles_0313.py [--apply]      (default = dry run)

WHERE THIS RUNS
---------------
**On the 8.6 host, as `lgsj`** (no sudo: everything lives under /home/lgsj).
All paths below are 8.6-absolute. Requires the 0.3.13 tarball already extracted to
/tmp/rel0313 (extract dist/dsh-raganything-kb-0.3.13-sharedkb.tgz there first).

Idempotent: a second run reports "dep already 0.3.13" for all 12 and changes nothing.
"""
import hashlib
import json
import os
...
REF = "/tmp/rel0313/package"
OLD_DEP = "file:/home/lgsj/dsh-raganything-kb-0.3.12-sharedkb.tgz"
NEW_DEP = "file:/home/lgsj/dsh-raganything-kb-0.3.13-sharedkb.tgz"
...
assert ref_pj["version"] == "0.3.13", ref_pj["version"]
...
for pkg in PROFILE_PKGS:
    ...
    if NEW_DEP in raw:
        rec["actions"].append("dep already 0.3.13 (skip)")
    elif raw.count(OLD_DEP) == 1:
        if APPLY:
            shutil.copy2(pkg, f"{pkg}.bak-rel0313-{TS}")
            open(pkg, "w", encoding="utf-8").write(raw.replace(OLD_DEP, NEW_DEP))
        rec["actions"].append("dep 0.3.12 -> 0.3.13")
    else:
        rec["ok"] = False
    ...
    if not os.path.isdir(pdir):
        ...
        if cur == ref_pj_bytes:
            rec["actions"].append("installed package.json already 0.3.13")
        else:
            ver = json.loads(cur.decode("utf-8")).get("version")
            if APPLY:
                shutil.copy2(ipj, f"{ipj}.bak-rel0313-{TS}")
                open(ipj, "wb").write(ref_pj_bytes)
            rec["actions"].append(f"installed package.json {ver} -> 0.3.13")
